...
int main() {
    ...
    do {
        ...
        else if (ch == 5) { int i; std::cout << "Индекс: "; std::cin >> i; std::cin.ignore(); keeper.edit(i); }
        else if (ch == 6) { int i; std::cout << "Индекс: "; std::cin >> i; std::cin.ignore(); keeper.remove(i); }
        else if (ch == 7) { std::string fn; std::cout << "Файл: "; std::getline(std::cin, fn); keeper.saveToFile(fn); }
        else if (ch == 8) { std::string fn; std::cout << "Файл: "; std::getline(std::cin, fn); keeper.loadFromFile(fn); }
    } while (ch != 9);
    return 0;
}
